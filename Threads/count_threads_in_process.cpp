# Count the amount of threads that a process has and what the base priority of a process is.
...
void EnumerateProcesses() {
    ...
    if (status == STATUS_INFO_LENGTH_MISMATCH) {
        ...
        processList = std::make_unique<BYTE[]>(length); // Buffer to hold information returned by NtQuerySystemInformation for SystemProcessInformation
    ...
    std::wcout << "PID | Process Name | Num of Threads | Base Priority\n";
    ...
    do {
        ...
        std::wcout << processId << " | " << processName.c_str() << " | "
                   << processEntry->NumberOfThreads << " | " << processEntry->BasePriority << std::endl;
